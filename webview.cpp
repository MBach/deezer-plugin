...
QWebPage *WebPage::createWindow(QWebPage::WebWindowType type)
{
	...
	WebPage *w = new WebPage(NULL);
	//w->setNetworkAccessManager(NetworkAccessManager::getInstance());
	return w;
}
...
void WebPage::handleUnsupportedContent(QNetworkReply *reply)
{
	...
	if (m_loadingUrl != reply->url()) {
		...
		qDebug() << "Resource" << reply->url().toEncoded() << "has unknown Content-Type, will be ignored.";
		reply->deleteLater();
		return;
	}
